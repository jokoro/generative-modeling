...
class DiffusionModel(nn.Module):
    def __init__(
        ...
        # previous timesteps.
        ##################################################################
        self.alphas_cumprod = torch.tensor([alphas[:i+1].prod() 
        ...
        self.x_0_pred_coef_2 = - self.x_0_pred_coef_1 * torch.sqrt(
        ...
        self.posterior_mean_coef1 = torch.sqrt(
            self.alphas_cumprod_prev)*self.betas/(1-self.alphas_cumprod)
        ...
        self.posterior_variance = (1-self.alphas_cumprod_prev
                                   )*self.betas/(1-self.alphas_cumprod)
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
    ...
    def predict_denoised_at_prev_timestep(self, x, t: int):
        ...
        (
            posterior_mean, posterior_variance, _
        ) = self.get_posterior_parameters(x_0, x, t)
        
        pred_img = posterior_mean + torch.sqrt(
            posterior_variance
        ) * torch.randn_like(posterior_mean)
    ...
    def ddim_step(self, batch, device, tau_i, tau_isub1, img, model_predictions, alphas_cumprod, eta):
        ...
        tau_i       = torch.full((batch,), tau_i, 
                                 device=device, dtype=torch.long)
        pred_noise, x_0 = model_predictions(img, tau_i)
        x_0 = x_0.clamp(-1.,1.)
        
        # if tau_isub1 < 0: return x_0, x_0
        tau_isub1   = torch.full((batch,), max(0,tau_isub1), 
                                 device=device, dtype=torch.long)
        

        # Step 2: Extract \alpha_{\tau_{i - 1}} and \alpha_{\tau_{i}}
        ex_alphas_cumprod       = extract(alphas_cumprod, tau_i, img.shape)
        ex_alphas_cumprod_prev  = extract(alphas_cumprod, tau_isub1, img.shape)
        ex_beta                 = extract(self.betas, tau_i, img.shape)

        # Step 3: Compute \sigma_{\tau_{i}}
        beta_tilde = (1-ex_alphas_cumprod_prev
                      )*ex_beta/(1-ex_alphas_cumprod)
        sigma_2 = eta * beta_tilde
        
        # Step 4: Compute the coefficient of \epsilon_{\tau_{i}}
        epsilon_coefficient = torch.sqrt( 1 
                                          - ex_alphas_cumprod_prev 
                                          - sigma_2 
                                        )

        # Step 5: Sample from q(x_{\tau_{i - 1}} | x_{\tau_t}, x_0)
        # HINT: Use the reparameterization trick
        mu = torch.sqrt(ex_alphas_cumprod_prev
                        ) * x_0 + epsilon_coefficient * pred_noise
        img = mu + torch.sqrt(sigma_2) * torch.randn_like(mu)
